...
class MapExporter(bpy.types.Operator, ImportHelper):
    ...
    def execute(self, context):
        ...
        visible_collections = [
            main_collection.get(BOTH_COLLECTION_NAME),
            main_collection.get(GRAPHICS_COLLECTION_NAME)
        ]
        collision_collections = [            
            main_collection.get(BOTH_COLLECTION_NAME),
            main_collection.get(COLLISION_COLLECTION_NAME)
        ]
        ...
        for mat, ob_indices in material_object_map.items():
            ...
            for j in ob_indices:
                ...
                write_object_to_mesh_data(ob, mesh_data)
        ...
        for collection in collision_collections:
            filepath = "%s/models/%s.ozt" % (directory, collection.name)
            save_ozyterrain(filepath, collection)
            print("Finished exporting %s.ozt" % collection.name)
